...
// Fully implemented, does not need to be edited
...
Decision getInformationGain(string att, vector<double> &dat, vector<int> &out, vector<int> &inst)
{
	...
	{									 // for the instances under consideration
		...
		{ // determine if this is a new possible decision or existing one
			if (pd.at(k).value == val)
			{ // exists, update the outcome instances
				exists = true;
				if (outc == 0)
				{
		...
		if (!exists)
		{ // create a new pre-decision point
			PreDecision npd;
			npd.value = val;
			if (outc == 0)
			{
